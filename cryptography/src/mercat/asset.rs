...
use crate::{
    ...
    mercat::{
        ...
        PubAccount, PubAssetTxData, PubAssetTxDataContent, SecAccount, TxSubstate,
        WellformednessProof,
    },
    Balance,
};
...
impl AssetTxIssueMediator {
    /// Called by the mediator during processing. It verifies the proofs generated by the issuer.
    /// TODO: This function doesn't need to be public, it can be private to the AssetTxIssueMediator
    /// and be called by `AssetTransactionMediator.justify_and_process()`.
    /// TODO: This function is not tested yet.
    pub fn verify(
        &self,
        asset_tx: &PubAssetTxData,
        issr_pub_account: &PubAccount,
        mdtr_enc_key: &EncryptionKeys,
    ) -> Fallible<()> {
        let gens = PedersenGens::default();

        // TODO: Does mediator need to validate the state when it gets here?
        // Validate the state.
        // ensure!(
        //     state == AssetTxState::Initialization(TxSubstate::Started),
        //     ErrorKind::InvalidPreviousAssetTransactionState { state }
        ...
        asset_issuance_init_verify(asset_tx, issr_pub_account, &mdtr_enc_key.pblc)?;

        // Mediator decrypts the encrypted amount and uses it to verify the correctness proof.
        let amount = mdtr_enc_key
            .scrt
            .key
