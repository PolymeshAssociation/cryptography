...
use private_identity_audit::{
    uuid_to_scalar, Challenge, ChallengeGenerator, ChallengeResponder, CommittedUids, FinalProver,
    InitialProver, PrivateUids, ProofGenerator, ProofVerifier, Proofs, ProverFinalResponse,
    ProverSecrets, Verifier, VerifierSecrets, VerifierSetGenerator,
};
use rand_core::OsRng;
...
pub fn greet() {
    ...
    let result1 = generate_initial_proofs(claim).unwrap();
    ...
    // let result = verify_proofs(result1.proofs, result3.response, challenge, cdd_id, result2.verifier_secrets, result3.committed_uids).is_ok();
...
// #[wasm_bindgen]
// pub fn convert_uuid_to_scalar(uuid: Base64) -> Base64 {
//     let uuid = decode::<Uuid>(uuid);
//     base64::encode(private_identity_audit::uuid_to_scalar(uuid).as_bytes())
// }

#[wasm_bindgen]
pub fn generate_initial_proofs(cdd_claim: Base64) -> Fallible<InitialProofsOutput> {
    let raw_cdd_data: RawCddClaimData =
        serde_json::from_str(&cdd_claim).map_err(|_| WasmError::DeserializationError)?;
