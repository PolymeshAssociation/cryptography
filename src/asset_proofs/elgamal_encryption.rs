...
mod tests {
    ...
    #[test]
    #[wasm_bindgen_test]
    fn play_with_homomorphic_encryption() {
        let v1: Scalar = 6u32.into();
        let v2: Scalar = 5u32.into();
        let mut rng = StdRng::from_seed(SEED_2);
        let r1 = Scalar::random(&mut rng);
        let r2 = Scalar::random(&mut rng);
        ...
        assert_eq!(cipher1, CipherText::default() + cipher1);
        let cipher2 = elg_pub.encrypt(&CommitmentWitness {
            value: v2,
            blinding: r2,
        });

        let mut cipher12 = cipher1 + cipher2;
        let value = elg_secret_key.decrypt(&cipher12);
        println!("value: {:?}", value);
        cipher12 -= cipher1;
        cipher12 -= cipher2;
        println!("result: {:?}", cipher12);
        println!("zero: {:?}", CipherText::default());
        // assert_eq!(cipher12, CipherText {x: RistrettoPoint::default(), y: RistrettoPoint::default()});
        assert_eq!(cipher12, CipherText::default());
        // let value = elg_secret_key.decrypt(&cipher12);
        // println!("value: {:?}", value);
        // cipher12 += cipher1;
        // assert_eq!(cipher12, cipher1);
