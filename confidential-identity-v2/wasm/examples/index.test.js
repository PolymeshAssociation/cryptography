...
const investor_did = [0x49, 0x99, 0x52, 0x43, 0x74, 0x8c, 0x4a, 0xe7, 0x11, 0x8, 0x3c, 0x97, 0x56, 0x5f, 0xfd, 0xfd, 0x60, 0xdb, 0x1d, 0x8c, 0xc5, 0x85, 0xf8, 0xa7, 0x1d, 0x99, 0x93, 0x9c, 0xbe, 0xab, 0xdd, 0x5];
const investor_unique_id = [0x96, 0xd2, 0x2c, 0x25, 0x4a, 0xe1, 0xf4, 0x44, 0xe1, 0x3c, 0x6d, 0x7f, 0xc6, 0xde, 0xc2, 0xca];
const scope_did = [0x8a, 0xe1, 0x49, 0xda, 0xb0, 0x2a, 0xa9, 0x8f, 0x7e, 0xd2, 0xe8, 0x2];

test('create CDD ID', () => {
  const cdd_claim = {"investor_did": investor_did, "investor_unique_id": investor_unique_id};
  const cdd_id_str = cil.create_cdd_id(JSON.stringify(cdd_claim));

  const got_cdd_id = JSON.parse(cdd_id_str);
  const want_cdd_id = [30, 129, 68, 184, 40, 28, 244, 188, 229, 174, 230, 87, 147, 91, 85, 31, 87, 221, 60, 110, 52, 4, 32, 196, 87, 59, 63, 99, 253, 118, 34, 41];
...
test('Generate Secure Seed', () => {
  ...
  const seed = buff.toJSON().data;

  const cdd_claim = {"investor_did": investor_did, "investor_unique_id": investor_unique_id};
  const scope_claim = {"scope_did": scope_did, "investor_unique_id": investor_unique_id};
  const proof = cil.create_scope_claim_proof(JSON.stringify(cdd_claim), JSON.stringify(scope_claim), JSON.stringify(seed));
...
test('verify the proof - positive case', () => {
  ...
  const cdd_id = cil.create_cdd_id(cdd_claim);
  const proof = cil.create_scope_claim_proof(cdd_claim, scope_claim, seed);
  expect(() => {
...
test('verify the proof - negative test', () => {
  ...
  const cdd_id = cil.create_cdd_id(cdd_claim);
  const proof_str = cil.create_scope_claim_proof(cdd_claim, scope_claim, seed);
