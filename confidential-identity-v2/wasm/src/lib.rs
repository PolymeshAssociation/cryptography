...
pub fn _verify_scope_claim_proof(
    ...
    use confidential_identity_v2::VerifierTrait;

    let proof: confidential_identity_v2::ScopeClaimProof = serde_json::from_str(&proof)
    ...
    let cdd_id: confidential_identity_v2::CddId = serde_json::from_str(&cdd_id)
        .map_err(|error| format!("Failed to deserialize the cdd_id: {}", error))?;

    confidential_identity_v2::claim_proofs::Verifier::verify_scope_claim_proof(
        &proof,
        &confidential_identity_v2::claim_proofs::slice_to_scalar(&investor_did),
        &confidential_identity_v2::claim_proofs::slice_to_scalar(&scope_did),
        &cdd_id,
    )
