...
fn justify_asset_issuance(cfg: input::JustifyIssuanceInfo) -> Result<(), Error> {
    ...
    save_object(
        ...
        ON_CHAIN_DIR,
        &cfg.issuer,
        &asset_transaction_file(cfg.tx_id, AssetTxState::Justification(TxSubstate::Started)),
        &next_instruction,
    )?;
...
fn justify_asset_transaction(cfg: input::JustifyTransactionInfo) -> Result<(), Error> {
    ...
    let instruction_path =
        confidential_transaction_file(cfg.tx_id, TxState::Finalization(TxSubstate::Validated));
